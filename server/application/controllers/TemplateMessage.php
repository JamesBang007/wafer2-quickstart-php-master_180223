...
if(!empty($postData['appname'])){
	//echo '1\n ';
	$appid = $app[$postData['appname']]['appid'];
	$appsecret = $app[$postData['appname']]['appsecret'];
}else{
	//echo '0\n ';
...
if( !empty($postData['formData']) ){
    $p1->send_template_message( $postData['formData'] );
}else{
	echo 'No FormData';
}
...
class TemplateMessage extends CI_Controller {
	...
	function send_template_message($data)
    {
        ...
        //echo "\n ".$url;
        $this->curl_request($url, json_encode($data));
    }
