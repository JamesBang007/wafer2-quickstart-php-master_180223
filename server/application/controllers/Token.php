...
class Token extends CI_Controller {
    public function index() {		
		...
		$running_time = date('Y-m-d H:i:s');
		//echo('时间2：'.$running_time);
		

		if( !empty($res) && time()<strtotime($res->last_visit_time)+7000 ){
			$msg = '有效，不更新 ';
			//print_r($res);
		}else{
			$msg = '过期，更新 ';
			...
			// 更新数据
			DB::update('cAccessToken', ['access_token' => $data['access_token'], 'last_visit_time' => $running_time], 'appid = "'.$appid.'"');
			$res = DB::row('cAccessToken', ['*'], 'appid = "'.$appid.'"');	// 查询一条 条件为字符串
			//print_r($res);
		}
